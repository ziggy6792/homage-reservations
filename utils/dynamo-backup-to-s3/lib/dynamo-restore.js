...
function DynamoRestore(options) {
  ...
  AWS.config.update({
    ...
    s3ForcePathStyle: options.endpoint ? true : undefined,
  });
      ...
      this.readline = readline
        ...
        .on(
          ...
          function () {
            ...
            const nextItems = this.requestItems.splice(0, DYNAMO_CHUNK_SIZE);
...
DynamoRestore.prototype._processLine = function (line) {
  ...
  if (this.requestItems.length > 0) {
    const itemTypes = this.requestItems.map((item) => item.PutRequest.Item.__typename.S);

    const uniqueItemTypes = itemTypes.filter((item, i, ar) => ar.indexOf(item) === i);

    if (uniqueItemTypes.length > 1) {
      throw new Error(`Should be only 1 item type but found ${uniqueItemTypes}`);
    }

    // console.log('uniqueItemTypes', uniqueItemTypes);

    const tableName = this.options.table.split('-').pop();

    const itemType = uniqueItemTypes[0];

    if (itemType !== tableName) {
      throw new Error(`For some fucked up reason I am trying to put '${itemType}' items in the '${tableName}' table`);
    }
  }
...
DynamoRestore.prototype._extractSchema = function (template) {
  ...
  if (partitionkey) {
    ...
    var likelyCandidates = Object.keys(template).filter(function (column) {
      return column.toLowerCase().substr(-2) === 'id' || column.toLowerCase().substr(-3) === 'key' || column.toLowerCase().substr(-3) === 'ref';
    });
